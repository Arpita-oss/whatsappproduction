...
io.on( "connection", function( socket ) {
    ...
    socket.on('private-message', async messageObject=>{
        ...
        const receiver = await user.findOne({
            username: messageObject.receiver,  //user dudhega agr null milega kyuki wo group m message kr rha h
        });
        ...
        if(!receiver){ //to ye query chlegi
            ...
            group.users.forEach(user=>{
                socket.to(user.socketId).emit('receive-private-message', messageObject)  //messageObject have message,sender,receiver
            })
